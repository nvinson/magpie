...
BDEPEND="
	|| ( sys-devel/gcc:14 sys-devel/gcc:13 sys-devel/gcc:12 sys-devel/gcc:11 )
	$(python_gen_any_dep '
...
src_configure() {
	if ! tc-is-gcc || ! has_version "<sys-devel/gcc-15"; then
		die "<=sys-devel/gcc:14 required"
	fi
	local slot
	for slot in 14 13 12 11; do
		if has_version "sys-devel/gcc:${slot}"; then
			export CC="${CHOST}-gcc-${slot}"
			export CXX="${CHOST}-g++-${slot}"
			break
		fi
	done
